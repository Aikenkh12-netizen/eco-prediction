...
# ←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←
GROQ_API_KEY = "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"   # ← твой ключ здесь
...
with col2:
    st.metric("Температура", f"{temp:.1f} °C")
with col3:
    st.metric("Мутность", f"{turb:.1f} NTU")
with col4:
    st.metric("Индекс SRI", f"{sri}/10", delta=round(sri - 7.0, 2))
...
with tab2:
    ...
    if photo:
        ...
        if st.button("🔍 Запустить анализ фото", type="primary"):
            with st.spinner("Groq анализирует изображение..."):
                # Пока простой ответ (можно потом улучшить)
                st.success("Анализ фото завершён")
                st.write("📸 **Результат анализа:** Мутность выглядит средней. Цвет воды слегка зеленоватый. Возможно присутствие органических веществ. Рекомендуется дополнительная проверка на месте.")
...
st.caption("SuVision Global AI • Работает на Groq • Прототип для мониторинга водоёмов Казахстана")
